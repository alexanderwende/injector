import { getParameterAnnotation, getPropertyAnnotation, setParameterAnnotation, setPropertyAnnotation } from '../annotations/index.js';
 ...
 */
export const inject = (injectToken) => {
    return (target, propertyKey, parameterIndex) => {
        const isParameterDecorator = typeof parameterIndex === 'number';
        const token = injectToken instanceof InjectToken
            ? injectToken
        ...
        if (isParameterDecorator) {
            setParameterAnnotation(target, parameterIndex, { token });
        }
        else {
            setPropertyAnnotation(target.constructor, propertyKey, { token });
        }
    };
};
