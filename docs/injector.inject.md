...
[Home](./index.md) &gt; [injector](./injector.md) &gt; [inject](./injector.inject.md)
...
```typescript
inject: <T>(injectToken?: InjectToken<T> | undefined) => (target: Object, propertyKey: string | number | symbol, parameterIndex?: number | undefined) => void
```

## Remarks

The inject decorator has two use cases: - Injecting dependencies into class properties - Injecting non-class dependencies through [InjectToken](./injector.injecttoken.md)<!-- -->s

When used to inject class properties, it can be used without specifying an [InjectToken](./injector.injecttoken.md)<!-- -->. The property type will be used as token and the injector will create a [ClassProvider](./injector.classprovider.md) for the type automatically.
...
```
When used to inject non-class properties, an [InjectToken](./injector.injecttoken.md) needs to be provided. An [InjectToken](./injector.injecttoken.md) can be used for property and parameter injection alike.
